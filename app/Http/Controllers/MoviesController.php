...
class MoviesController extends Controller
{
    ...
    public function movies($type=1){
        $genres=Http::get('https://api.themoviedb.org/3/genre/movie/list?api_key='.config('api_key',''))->json();
        $genre=collect($genres['genres'])->mapWithKeys(function($genre){
            return [$genre['id']=>$genre['name']];
        });
        if($type==1){
            $popularmovies=Http::get('https://api.themoviedb.org/3/movie/popular?api_key='.config('api_key',''))->json();
            return view('movies')->with('popularmovies',$popularmovies['results'])
                                ->with('genres',$genre);
        }
        elseif($type==2){
            $topratedmovies=Http::get('https://api.themoviedb.org/3/movie/top_rated?api_key='.config('api_key',''))->json();
            //dd($popularmovies);
            ...
            $nowplayingmovies=Http::get('https://api.themoviedb.org/3/movie/now_playing?api_key='.config('api_key',''))->json();
    ...
    public function show($id){
        $moviedetails=Http::get('https://api.themoviedb.org/3/movie/'.$id.'?api_key='.config('api_key','').'&append_to_response=credits,videos,keywords,recommendations,external_ids')->json();
        //dd($moviedetails);
    ...
    public function movie_images($id){
        $images=Http::get('https://api.themoviedb.org/3/movie/'.$id.'/images?api_key='.config('api_key',''))->json();
        //dd($images);
    ...
    public function search(Request $request){
        $search=Http::get('https://api.themoviedb.org/3/search/multi?api_key='.config('api_key','').'&query='.$request->search.'')->json();
