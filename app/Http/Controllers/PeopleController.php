...
class PeopleController extends Controller
{
    public function peoples($page=1){
        $peoples=Http::get('https://api.themoviedb.org/3/person/popular?page='.$page.'&api_key='.config('api_key',''))->json();
        if(request()->ajax()){
    ...
    public function people_show($id){
        $people=Http::get('https://api.themoviedb.org/3/person/'.$id.'?api_key='.config('api_key','').'&append_to_response=movie_credits,tv_credits,external_ids,combined_credits')->json();    
