<ul class="absolute mt-1 w-full border border-gray-300 bg-gray-800 border_list overflow-auto">
    ...
    <li class=" py-1 text-white flex text-sm hover:bg-gray-700">
        @if($sea['media_type'] == 'person')
            <a href="{{route('people.show', $sea['id'])}}">
        ...
        @elseif($sea['media_type'] == 'movie')
            <a href="{{route('movie.show', $sea['id'])}}">
        ...
        @endif
            @if($sea['media_type'] == 'person')
                <a href="{{route('people.show', $sea['id'])}}" class="align-top mr-2 text-gray-200 flex items-center">{{$sea['name']}}</a>
            @elseif($sea['media_type'] == 'movie')
                <a href="{{route('movie.show', $sea['id'])}}" class="align-top mr-2 text-gray-200 flex items-center">{{$sea['title']}}</a>
            @else
                <a href="{{route('show.show', $sea['id'])}}" class="align-top mr-2 text-gray-200 flex items-center">{{$sea['name']}}</a>
            @endif
    </li>
    @endforeach
</ul>